...
import {
  ...
  QueuesAckAllMessagesRequest,
  QueuesPullWaitingMessagesRequest,
} from '../../src/queues/queuesTypes';
...
describe('QueuesClient Integration Test', () => {
  ...
  it('should send a queue message', async () => {
    const mockSendQueueMessage = jest
      .spyOn(client.grpcClient as any, 'sendQueueMessage')
      .mockImplementation(
        (request: any, metadata: any, options: any, callback: any) => {
  ...
  it('should delete a queue channel', async () => {
    const mockDeleteChannel = jest
      .spyOn(client['grpcClient'] as any, 'deleteChannel')
      .mockImplementation((request: any, metadata: any, callback: any) => {
    ...
    await expect(
      client.deleteQueuesChannel('test-channel'),
  ...
  it('should list queue channels', async () => {
    const mockListChannels = jest
      .spyOn(client.grpcClient as any, 'listQueuesChannels')
      .mockImplementation((request: any, metadata: any, callback: any) => {
        const response = new pb.kubemq.Response();
        response.Executed = true;
  ...
  it('should waiting queue messages', async () => {
    const mockPeekMessages = jest
      .spyOn(client.grpcClient as any, 'receiveQueueMessages')
      .mockImplementation((request: any, metadata: any, callback: any) => {
    ...
    const queuePollRequest: QueuesPullWaitingMessagesRequest = {
  ...
  it('should receive queue messages', async () => {
    const mockReceiveMessages = jest
      .spyOn(client.grpcClient as any, 'receiveQueueMessages')
      .mockImplementation((request: any, metadata: any, callback: any) => {
        const response = new pb.kubemq.ReceiveQueueMessagesResponse();
        const message = new pb.kubemq.QueueMessage();
    ...
    const pullQueueMessageRequest: QueuesPullWaitingMessagesRequest = {
      id: 'unique-request-id',
      channel: 'my-channel',
      clientId: 'my-client-id',
      maxNumberOfMessages: 10,
