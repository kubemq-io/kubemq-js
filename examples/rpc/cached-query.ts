...
async function main(): Promise<void> {
  const client = await KubeMQClient.create({ address: 'localhost:50000', clientId: 'js-rpc-cached-query-client' });
  ...
  try {
      ...
      onQuery: (q) => {
        ...
        client.sendQueryResponse({
          ...
          body: new TextEncoder().encode(
            JSON.stringify({ sku: 'WIDGET-42', qty: 128 }),
          ),
        });
      },
      onError: (err) => {
    ...
    console.log('Response 1 — executed:', response1.executed, '| cacheHit:', response1.cacheHit ?? false);
    ...
    console.log('Response 2 — executed:', response2.executed, '| cacheHit:', response2.cacheHit ?? false);
