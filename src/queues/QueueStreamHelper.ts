  ...
  public async sendMessage(
    ...
    return new Promise<QueueMessageSendResult>((resolve, reject) => {
      if (!this.queuesUpStreamHandler) {
        ...
        const duplexStream: grpc.ClientDuplexStream<
          pb.kubemq.QueuesUpstreamRequest,
          pb.kubemq.QueuesUpstreamResponse
        > = kubeMQClient.grpcClient.QueuesUpstream();

        // Listen for data (responses from the server)
        duplexStream.on(
          'data',
          (messageReceive: pb.kubemq.QueuesUpstreamResponse) => {
            ...
            if (!messageReceive.IsError) {
              const qsr: QueueMessageSendResult = {
                ...
                sentAt:
                  messageReceive.Results[0].SentAt > 0
                    ? messageReceive.Results[0].SentAt / 1000000000
                    : 0,
                expirationAt:
                  messageReceive.Results[0].ExpirationAt > 0
                    ? messageReceive.Results[0].ExpirationAt / 1000000000
                    : 0,
                delayedTo:
                  messageReceive.Results[0].DelayedTo > 0
                    ? messageReceive.Results[0].DelayedTo / 1000000000
                    : 0,
  ...
  public async receiveMessage(
    ...
    return new Promise<QueuesMessagesPulledResponse>((resolve, reject) => {
      if (!this.queuesDownstreamHandler) {
        // Initiate the duplex stream for both sending and receiving messages
        const duplexStream: grpc.ClientDuplexStream<
          pb.kubemq.QueuesDownstreamRequest,
          pb.kubemq.QueuesDownstreamResponse
        > = kubeMQClient.grpcClient.QueuesDownstream();

        // Listen for responses (data) from the server
        duplexStream.on(
          'data',
          (messageReceive: pb.kubemq.QueuesDownstreamResponse) => {
            console.log(
              `QueuesDownstreamResponse Received->: ${messageReceive}`,
            );
      ...
      if (this.queuesDownstreamHandler) {
        const result = this.queuesDownstreamHandler.write(queuesPollRequest);
        if (!result) {
          reject('Failed to write to QueuesDownstreamRequest stream.');
        }
      }
    });
  }
