/**
 * Commands worker: RPC request/response via sendCommand() + subscribeToCommands().
...
export class CommandsWorker extends BaseWorker {
  ...
  startConsumers(client: KubeMQClient): void {
      ...
      const sub = client.subscribeToCommands(
        {
          ...
          onCommand: async (cmd) => {
            ...
            try {
              await client.sendCommandResponseDirect({
                ...
                executed: true,
                body: isWarmup ? undefined : cmd.body,
              });
            } catch (e) {
          ...
          onError: (err) => {
            console.error(`commands subscription error (ch ${chIdx}): ${err.message}`);
    ...
    console.log(`commands responders started on ${this.channelName}`);
  }
  ...
  startProducers(client: KubeMQClient): void {
    ...
    console.log(`commands senders started on ${this.channelName}`);
  }
      ...
      const task = (async () => {
        try {
          ...
          if (resp.error) {
            ...
            mc.incReceived(PATTERN, senderId, body.length);
          }
        } catch (e: any) {
