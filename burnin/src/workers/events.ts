/**
 * Events worker: fire-and-forget pub/sub via createEventStream() + subscribeToEvents().
 * v2: channelName, rate, channelIndex passed in; worker IDs include channel index.
 */
import type { KubeMQClient, Subscription, EventStreamHandle } from 'kubemq-js';
...
export class EventsWorker extends BaseWorker {
  ...
  private streamHandle: EventStreamHandle | null = null;
  ...
  startConsumers(client: KubeMQClient): void {
      ...
      const sub = client.subscribeToEvents(
        {
          ...
          onError: (err) => {
            console.error(`events subscription error (ch ${chIdx}): ${err.message}`);
    ...
    console.log(`events consumers started on ${this.channelName}`);
  }

  startProducers(client: KubeMQClient): void {
    this.streamHandle = client.createEventStream();
    ...
    console.log(`events producers started on ${this.channelName}`);
  }
