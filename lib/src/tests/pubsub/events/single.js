...
function main() {
    ...
    const receiverSettings = new classes_1.Settings('localhost:50000').setClientId('receiver-client-id');
    ...
    const subscriber = receiver.subscribeToEvents(new classes_2.EventsSubscriptionRequest('events.single', ''), receiveHandler, errorHandler, stateHandler);
    setTimeout(() => {
        for (let i = 0; i < 10; i++) {
