...
function main() {
    const senderSettings = new classes_1.Settings('localhost:50000').setClientId('sender-client-id');
    const receiverSettingsA = new classes_1.Settings('localhost:50000').setClientId('receiver-client-id-a');
    const receiverSettingsB = new classes_1.Settings('localhost:50000').setClientId('receiver-client-id-b');
    ...
    const subscriberA = receiverA.subscribeToEvents(new classes_2.EventsSubscriptionRequest('events.single', 'group1'), receiveAHandler, errorAHandler, stateAHandler);
    ...
    const subscriberB = receiverB.subscribeToEvents(new classes_2.EventsSubscriptionRequest('events.single', 'group1'), receiveBHandler, errorBHandler, stateBHandler);
    setTimeout(() => {
        for (let i = 0; i < 10; i++) {
